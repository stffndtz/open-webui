...
class WordEditor:
    ...
    def read_document(self, event_emitter=None) -> dict:
        ...
        for i, element in enumerate(all_elements):
            if isinstance(element, Paragraph):
                ...
                log.debug(
                    f"Index {i:2d}: Block {block_id:12s} | Style: {style_name:15s} | Text: {text_preview}"
                )
            else:
                block_id = self.element_to_id.get(element, "unknown")
    ...
    def edit_block(self, block_id: str, new_text: str):
        ...
        if isinstance(element, Paragraph):
            ...
            return {
                "status": "success",
                "message": f"Block '{block_id}' updated with preserved formatting.",
            }
        else:
    ...
    def insert_block(self, relative_to_block_id: str, position: str, new_block: dict):
        ...
        if new_block["type"] == "paragraph":
            ...
            if isinstance(relative_element, Paragraph):
                log.debug(
                    f"insert_block - relative_element is a paragraph: '{relative_element.text[:30]}...'"
                )
                ref_style = (
                    relative_element.style.name
                    if relative_element.style
                    else "No style"
                )
                log.debug(f"insert_block - relative_element style: '{ref_style}'")

                # Always use the reference element's style (the one we're inserting next to)
                # This ensures the new paragraph matches its logical sibling, not an unrelated next section
                log.debug(
                    f"insert_block - Using reference element for styling: '{relative_element.text[:30]}...'"
                )
            ...
            if isinstance(relative_element, Paragraph):
                ...
                ref_text = (
                    relative_element.text[:50] + "..."
                    if len(relative_element.text) > 50
                    else relative_element.text
                )
                ...
                for element, block_id in self.element_to_id.items():
                    if (
                        hasattr(element, "_element")
                        and element._element == relative_element._element
                    ):
                ...
                log.debug(f"Style Source (Reference Element):")
                ...
                log.debug(
                    f"Reference Element: Not a paragraph (type: {type(relative_element)})"
                )
    ...
    def insert_block_with_saved_style(
        self, relative_to_block_id: str, position: str, new_block: dict, style_name: str
    ):
        ...
        if new_block["type"] == "paragraph":
            ...
            if style_name in self._saved_styles:
                ...
                log.debug(
                    f"    Paragraph Style: {saved_style.get('paragraph_style', 'None')}"
                )
            ...
            return {
                "status": "success",
                "message": f"Block inserted with saved style '{style_name}'.",
            }
        else:
            # Support for tables and headings can be added here
            raise NotImplementedError("Only paragraph insertion is supported for now.")
    ...
    def _find_next_paragraph(self, reference_element):
        ...
        log.debug(
            f"_find_next_paragraph called for element: {reference_element.text[:50]}..."
        )
        ...
        ref_style = (
            reference_element.style.name if reference_element.style else "No style"
        )
        ...
        for i in range(ref_index + 1, len(all_elements)):
            ...
            if isinstance(element, Paragraph):
                ...
                log.debug(
                    f"Found paragraph at index {i}: style='{element_style}', text='{element.text[:30]}...'"
                )
    ...
    def _copy_paragraph_styles(
        self, source_paragraph: Paragraph, target_paragraph: Paragraph
    ):
    ...
    def apply_saved_styles(
        self, target_paragraph: Paragraph, style_name: str = "default"
    ):
    ...
    def get_styling_reference_info(self, block_id: str):
        ...
        if style_reference:
            ...
            return {
                ...
                "reference_text": (
                    style_reference.text[:50] + "..."
                    if len(style_reference.text) > 50
                    else style_reference.text
                ),
                "reference_style": (
                    style_reference.style.name if style_reference.style else "Normal"
                ),
    ...
    def debug_document_structure(self):
        ...
        return {
            "status": "success",
            "message": "Document structure logged to debug console",
        }
...
class Tools:
    ...
    async def _get_docx_file_path(
        ...
        log.debug(
            f"_get_docx_file_path: __files__={__files__}, file_name='{file_name}'"
        )
    ...
    async def get_edited_document(
        ...
        """
        Gets the current in memory state of the document for validation of changes
        """
        if __event_emitter__:
            await __event_emitter__(
                {
                    "type": "status",
                    "data": {
                        "description": f"Getting edited document {file_name}",
                        "done": False,
                        "hidden": False,
                    },
                }
    ...
    async def save_paragraph_styles(
        self,
        block_id: str = Field(
            ..., description="The ID of the block to save styles from."
        ),
    ...
    async def get_styling_reference_info(
        self,
        block_id: str = Field(
            ..., description="The ID of the block to check styling reference for."
        ),
        file_name: str = Field(
            default="",
            description="Name of the DOCX file (optional if file is uploaded).",
    ...
    async def upload_file_direct(
        ...
        try:
            ...
            log.debug(
                f"upload_file_direct: Storage upload successful, path: {file_path}"
            )
            ...
            log.debug(
                f"upload_file_direct: Success! File available at /api/v1/files/{file_id}"
            )
            return result
    ...
    def _place_file_in_openwebui_system(self, file_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            log.debug(
                f"_place_file_in_openwebui_system: Target file size: {os.path.getsize(target_path)} bytes"
            )
            ...
            log.debug(
                f"_place_file_in_openwebui_system: Success! File available at {target_path}"
            )
            return result
    ...
    async def generate_redline_document(
        ...
        try:
            ...
            log.debug(
                f"Redline saved. Size: {os.path.getsize(redline_output_path)} bytes"
            )
