			...
			await microsoftTeams.app
				...
				.then(async () => {
					try {
						...
						toast.success('Success - close popup now please');
					} catch {
						console.log('got already token from popup and its already closed');
						toast.error('Got already token from popup and its already closed');
					}
				})
				.catch(() => {
					console.log('no teams');
					toast.error('No Teams environment detected');
				});
		} catch (error) {
			console.log('Not in Teams environment or Teams SDK not available:', error);
			toast.error(`Not in Teams environment or Teams SDK not available: ${error}`);
		}
