"""
title: NDA Redline Tool 3.0
author: 42frontiers
author_url: https://42frontiers.com
version: 3.0.0
license: MIT
description: A tool for analyzing and redlining NDA documents. Reads DOCX files, allows block-based and table cell editing, and generates tracked-changes (redline) documents. Supports bilingual (German/English) table-based NDAs.
...
class _WordEditor:
    ...
    def __init__(self, file_path: str):
        ...
        # Table cell support for bilingual NDAs
        self._tables: List[Table] = []  # Ordered list of tables
        self._id_to_cell: Dict[str, tuple] = {}  # cell_id -> (table, row, col)
        self._cell_to_id: Dict[tuple, str] = {}  # (table_idx, row, col) -> cell_id
        self._table_metadata: Dict[int, dict] = {}  # table_idx -> classification
    ...
    def _build_block_map(self) -> None:
        ...
        for child in self.document.element.body:
            if isinstance(child, CT_P):
                ...
                self._tables.append(table)  # Also track in ordered list

    def _classify_tables(self) -> None:
        """
        Classify tables as content (editable bilingual) or metadata (forms/signatures).
        Content tables have 2+ columns with substantial text - typically bilingual NDAs.
        """
        for idx, table in enumerate(self._tables):
            ...
            classification = {
                ...
                "columns": cols,
                "rows": rows,
                "language_columns": None,
                "has_merged_cells": False,
            }

            # Content table heuristics: >10 rows and 2+ columns with substantial text
            if rows > 10 and cols >= 2:
                col_text = {}
                has_merged = False

                for row in table.rows[:min(15, rows)]:
                    seen_in_row = set()
                ...
                classification["has_merged_cells"] = has_merged

                # Find columns with substantial text (>500 chars in sample)
                text_cols = [i for i, length in col_text.items() if length > 500]

                if len(text_cols) >= 2:
                    classification["type"] = "content"
                    classification["language_columns"] = self._detect_language_columns(table, text_cols)
                    log.debug(f"Table {idx} classified as CONTENT: {rows} rows, lang_cols={classification['language_columns']}")

            self._table_metadata[idx] = classification

    def _detect_language_columns(self, table, text_cols: List[int]) -> Dict[int, str]:
        """
        Detect which column contains which language using keyword analysis.
        Returns dict mapping column index to language code ('de' or 'en').
        """
    ...
    def _map_table_cells(self) -> None:
        """
        Map individual cells in content tables for editing.
        Only maps cells in language columns of content tables.
        """
        ...
        log.debug(f"Mapped {len(self._id_to_cell)} table cells")
    ...
    def _flag_compliance_cells(self, cells: List[Dict]) -> Dict[str, List[Dict]]:
        """
        Pre-scan cells for compliance-relevant keywords.
        Returns flagged cells organized by requirement number.

        This helps GPT-4o find all relevant cells without scanning 100+ cells.
        """
        # Keywords by requirement and language
        keywords = {
            "req_8_immediately": {
                "de": {
                    "non_compliant": ["unverzüglich"],
                    "compliant": ["ohne schuldhaftes zögern"],
                },
                "en": {
                    "non_compliant": ["immediately"],
                    "compliant": ["without undue delay"],
                },
                "fr": {
                    "non_compliant": ["immédiatement"],
                    "compliant": ["sans délai injustifié", "sans délai indu"],
                },
                "es": {
                    "non_compliant": ["inmediatamente"],
                    "compliant": ["sin demora indebida"],
                },
            },
            "req_10_shall_ensure": {
                "de": {
                    "non_compliant": ["sicherstellen", "sicherzustellen", "stellt sicher"],
                    "compliant": ["anweisen", "anzuweisen"],
                },
                "en": {
                    "non_compliant": ["shall ensure", "will ensure", "must ensure"],
                    "compliant": ["shall instruct"],
                },
            },
        }
        ...
        for cell in cells:
            ...
            for req_key, lang_keywords in keywords.items():
                ...
                # Check for non-compliant terms
                for term in non_compliant:
                    if term in cell_text:
                        # Check if compliant term is also present (might be a replacement)
                        ...
                        flagged[req_key].append({
                            ...
                            "quote": cell_text[:100] + "..." if len(cell_text) > 100 else cell_text,
                        })
                        break  # Only flag once per cell per requirement

                # Also flag compliant cells so model knows they're OK
    ...
    def _classify_table(
        self, table_data: List[List[str]], row_count: int, col_count: int
    ) -> Dict[str, Any]:
        """
        ...
        if col_count in (2, 3) and row_count > 5:
            ...
            avg_text_length = sum(
                len(cell) for row in table_data for cell in row
            ) / max(1, row_count * col_count)
    ...
    def get_structured_content(self) -> Dict[str, Any]:
        ...
        for element, block_id in self._element_to_id.items():
            ...
            if isinstance(element, Paragraph):
                ...
                # Get classification from our enhanced table metadata
                table_idx = self._tables.index(element) if element in self._tables else -1
                if table_idx >= 0 and table_idx in self._table_metadata:
                    metadata = self._table_metadata[table_idx]
                    block_info["is_metadata"] = metadata["type"] == "metadata"
                    block_info["is_content_table"] = metadata["type"] == "content"

                    if metadata["type"] == "content":
                        block_info["language_columns"] = metadata["language_columns"]
                        block_info["has_merged_cells"] = metadata["has_merged_cells"]
                        block_info["note"] = (
                            "✅ BILINGUAL CONTENT TABLE - Editable via cell IDs (cell_X_Y_Z format)"
                        )
                    else:
                        block_info["note"] = (
                            "Metadata table (addresses/signatures) - can be ignored"
                        )
                else:
                    # Fallback to old classification for tables not in our list
                    table_data = [[cell.text for cell in row.cells] for row in element.rows]
                    classification = self._classify_table(table_data, row_count, col_count)
                    block_info["is_metadata"] = classification["is_metadata"]
                    block_info["is_content_table"] = not classification["is_metadata"]
        ...
        # Build table metadata for response
        tables_info = []
        content_table_indices = []
        metadata_table_indices = []
        ...
        for table_idx, metadata in self._table_metadata.items():
            ...
            if metadata["type"] == "content":
                content_table_indices.append(table_idx)
                table_info["language_columns"] = metadata["language_columns"]
                table_info["has_merged_cells"] = metadata["has_merged_cells"]
            else:
                metadata_table_indices.append(table_idx)
        ...
        # Determine document type based on table content and languages
        document_type = "paragraph_based"
        is_bilingual = False
        detected_languages = set()

        if content_table_indices:
            # Check if content tables are actually bilingual
            for table_idx in content_table_indices:
                lang_cols = self._table_metadata[table_idx].get("language_columns", {})
                detected_languages.update(lang_cols.values())

            is_bilingual = "de" in detected_languages and "en" in detected_languages

            if is_bilingual:
                document_type = "bilingual_table"
            else:
                document_type = "table_based"
                log.debug(f"Table NDA detected with languages: {detected_languages}")

        # Build summary
        paragraph_blocks = [
            b for b in blocks if b["type"] in ("paragraph", "heading", "list_item")
        ]

        summary = {
            "can_proceed": True,  # Always true now - we support all types
            ...
            "metadata_tables": metadata_table_indices,
            "total_cells": len(cells),
        }

        # Pre-flag cells containing compliance-relevant keywords
        # This helps the model find all cells that need review
        flagged_cells = self._flag_compliance_cells(cells)

        # Return with flagged_cells FIRST (most important for model)
        # This ensures it's not truncated if response is too long
        return {
            "flagged_cells": flagged_cells,  # FIRST - critical for compliance
            "summary": summary,
    ...
    def find_replace_in_block(
        ...
        find_text_normalized = self._normalize_quotes(find_text)
        current_text_normalized = self._normalize_quotes(current_text)

        # Check if find_text exists (check both original and normalized)
        use_normalized = False
        occurrences = current_text.count(find_text)
        if occurrences == 0:
            # Try normalized matching
            occurrences = current_text_normalized.count(find_text_normalized)
            if occurrences > 0:
                use_normalized = True
                log.debug(f"Using normalized matching for '{find_text[:30]}' in {block_id}")
            else:
                raise ValueError(
        ...
        for run in element.runs:
            ...
            run_text = run.text
            run_text_normalized = self._normalize_quotes(run_text) if use_normalized else run_text
            search_text = find_text_normalized if use_normalized else find_text

            if search_text in run_text_normalized:
                # Count how many we can replace in this run
                run_occurrences = run_text_normalized.count(search_text)
                can_replace = min(run_occurrences, max_replacements - replaced_count)

                if replace_all or can_replace > 0:
                    if use_normalized:
                        # Need to find the actual position in the original text and replace
                        start_idx = run_text_normalized.find(search_text)
                        if start_idx >= 0:
                            end_idx = start_idx + len(search_text)
                            run.text = run_text[:start_idx] + replace_text + run_text[end_idx:]
                            replaced_count += 1
                    else:
        ...
        if replaced_count == 0 and (find_text in current_text or (use_normalized and find_text_normalized in current_text_normalized)):
            # Text spans runs - need to do full paragraph replacement
            # This is less ideal but necessary for cross-run matches
            if use_normalized:
                # Use position-based replacement for normalized matching
                start_idx = current_text_normalized.find(find_text_normalized)
                if start_idx >= 0:
                    end_idx = start_idx + len(find_text_normalized)
                    new_text = current_text[:start_idx] + replace_text + current_text[end_idx:]
                else:
                    new_text = current_text  # Shouldn't happen, but be safe
            else:
                new_text = current_text.replace(
                    find_text, replace_text, 1 if not replace_all else -1
                )
    ...
    def insert_block(
        self, reference_block_id: str, position: str, text: str
    ) -> Dict[str, str]:
        """
        ...
        Also accepts cell IDs (cell_X_Y_Z) - will insert relative to the
        containing table block.
        ...
        if reference_block_id.startswith("cell_"):
            ...
            if len(parts) < 4:
                raise ValueError(f"Invalid cell ID format '{reference_block_id}'. Expected cell_TABLE_ROW_COL")
            table_idx = int(parts[1])
            if table_idx >= len(self._tables):
                # Provide helpful info about available tables
                available_tables = [f"block_t_{i}" for i in range(len(self._tables))]
                raise ValueError(
                    f"Table index {table_idx} out of range. Document has {len(self._tables)} tables. "
                    f"Available table blocks: {available_tables}"
                )
            ...
            if row_idx >= len(table.rows):
                # Suggest the last valid row
                last_row = len(table.rows) - 1
                raise ValueError(
                    f"Row {row_idx} out of range for table {table_idx} (has {len(table.rows)} rows). "
                    f"Try using cell_{table_idx}_{last_row}_0 instead, or use the table block directly."
                )

            table_block_id = self._element_to_id.get(table)
            if table_block_id:
                log.debug(f"insert_block: Converted cell ID '{reference_block_id}' to table block '{table_block_id}'")
                reference_block_id = table_block_id
            else:
                raise ValueError(f"Could not find table block for cell '{reference_block_id}'. Table not in block map.")
    ...
    # -------------------------------------------------------------------------
    # Table Cell Editing Methods (for bilingual NDAs)
    # -------------------------------------------------------------------------

    @staticmethod
    def _normalize_quotes(text: str) -> str:
        """Normalize Unicode smart quotes to ASCII equivalents for matching."""
        replacements = {
            '\u2018': "'",  # Left single quote
            '\u2019': "'",  # Right single quote
            '\u201C': '"',  # Left double quote
    ...
    def find_replace_in_cell(
        self, cell_id: str, find_text: str, replace_text: str
    ) -> Dict[str, Any]:
        """
        Find and replace text within a specific table cell.
        Preserves all formatting by operating at the run level.
        Handles Unicode smart quotes automatically.

        Args:
            cell_id: Cell ID in format 'cell_T_R_C' (table, row, column)
        ...
        # Normalize quotes in find_text for matching
        find_text_normalized = self._normalize_quotes(find_text)
        ...
        for para in cell.paragraphs:
            for run in para.runs:
                ...
                if find_text in run.text:
                    ...
                    replaced = True
                elif find_text_normalized in self._normalize_quotes(run.text):
                    # Find the actual text in run (with Unicode) and replace
                    run.text = run.text.replace(
                        run.text[self._normalize_quotes(run.text).find(find_text_normalized):
                                 self._normalize_quotes(run.text).find(find_text_normalized) + len(find_text_normalized)],
                        replace_text
                    )
                    replaced = True

        if not replaced:
            # Try full paragraph text (handles text split across runs)
            for para in cell.paragraphs:
                para_text = para.text
                para_text_normalized = self._normalize_quotes(para_text)

                if find_text in para_text or find_text_normalized in para_text_normalized:
                    # Replace in the original text
                    if find_text in para_text:
                        full_text = para_text.replace(find_text, replace_text)
                    else:
                        # Need to handle Unicode replacement
                        # Find position in normalized text, then replace in original
                        start_idx = para_text_normalized.find(find_text_normalized)
                        if start_idx >= 0:
                            end_idx = start_idx + len(find_text_normalized)
                            full_text = para_text[:start_idx] + replace_text + para_text[end_idx:]
                        else:
                            continue
        ...
        if not replaced:
            return {
                "status": "warning",
                "message": f"Text '{find_text}' not found in cell '{cell_id}'",
            }

        return {
            "status": "success",
            "message": f"Replaced '{find_text}' with '{replace_text}' in {cell_id}",
        }

    def get_corresponding_cell(self, cell_id: str) -> Optional[str]:
        """
        Get the cell ID of the corresponding language column for bilingual edits.
        Returns None if no corresponding cell exists.
        """
        if cell_id not in self._id_to_cell:
            return None
        ...
        table_idx, row_idx, col_idx = int(parts[1]), int(parts[2]), int(parts[3])

        metadata = self._table_metadata.get(table_idx)
        if not metadata or metadata["type"] != "content":
            return None

        lang_cols = metadata.get("language_columns", {})
        other_cols = [c for c in lang_cols.keys() if c != col_idx]

        if other_cols:
            other_cell_id = f"cell_{table_idx}_{row_idx}_{other_cols[0]}"
            if other_cell_id in self._id_to_cell:
                return other_cell_id
    ...
    def get_cell_info(self, cell_id: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "row": row_idx,
            "column": col_idx,
            "language": metadata["language_columns"].get(col_idx),
            "text": cell.text,
            "paragraph_count": len(cell.paragraphs),
            "corresponding_cell": self.get_corresponding_cell(cell_id),
        }
    ...
    def _copy_paragraph_formatting(self, source: Paragraph, target: Paragraph) -> None:
    ...
    def _apply_numbering_to_paragraph(
        self, paragraph: Paragraph, numId: int, ilvl: int = 0
    ) -> None:
        """
    ...
    def _find_surrounding_numbering(
        self, reference_block_id: str
    ) -> Optional[Dict[str, Any]]:
        """
    ...
    def insert_block_with_style(
        ...
        if style_from_block_id:
            if style_from_block_id not in self._id_to_element:
                raise ValueError(
                    f"Style source block '{style_from_block_id}' not found"
                )
    ...
    def get_changes_summary(self, original_path: str) -> Dict[str, Any]:
        ...
        changes = {"modified": [], "added": [], "deleted": [], "unchanged": 0}
        ...
        for block_id, original in original_content.items():
            if block_id in current_blocks:
                ...
                if (
                    ...
                    and current.get("text") != original["text"]
                ):
                    changes["modified"].append(
                        {
                            ...
                            "original": (
                                original["text"][:100] + "..."
                                if len(original["text"]) > 100
                                else original["text"]
                            ),
                            "current": (
                                current.get("text", "")[:100] + "..."
                                if len(current.get("text", "")) > 100
                                else current.get("text", "")
                            ),
                        }
                    )
                ...
                changes["deleted"].append(
                    {
                        "block_id": block_id,
                        "type": original["type"],
                        "content_preview": original.get(
                            "text", str(original.get("data", ""))
                        )[:50],
                    }
                )
...
class Tools:
    ...
    Supported Document Types:
        - paragraph_based: Standard paragraph-format NDAs
        - bilingual_table: Table-based NDAs with side-by-side language columns (e.g., DE/EN, FR/ES)
        - table_based: Table-based NDAs with a single language

    Workflow:
        1. read_document() - Load and parse the NDA, get document_type and block/cell IDs
        2. For paragraph documents: find_replace() / insert_block()
           For table documents: find_replace_in_cell() / insert_block()
        3. get_edited_document() - Validate changes (optional)
        4. generate_redline_document() - Create tracked-changes output

    Key Features:
        - Automatic document type classification (paragraph vs table-based)
        - Language detection for bilingual table NDAs (currently supports German/English detection)
        - Cell-level editing preserving formatting in table documents
        - Redline generation with tracked changes via XmlPowerToolsEngine

    For bilingual_table documents:
        - Each compliance issue requires edits in ALL language columns
        - Use cell IDs like cell_3_38_0 (col 0) and cell_3_38_1 (col 1)
        - The summary.detected_languages field shows which languages were detected
        - Check cells[] array for language tags on each cell
    """
    ...
    class Valves(BaseModel):
        ...
        REDLINE_SUFFIX: str = Field(
            default="_redlined",
            description="Suffix to append to the original filename for the redlined output (e.g., '_redlined', '_NH_markup').",
        )
    ...
    async def _get_docx_path(
        self, files: Optional[List[Dict]], file_name: Optional[str] = None
    ) -> str:
        """
        ...
        """
        log.debug(f"_get_docx_path called with files={files}, file_name={file_name}")

        # Check uploaded files first
        if files:
            log.debug(f"Processing {len(files)} files")
            for file_info in files:
                log.debug(f"File info: {file_info}")
                ...
                if name.lower().endswith(".docx"):
                    ...
                    if os.path.exists(path):
                        log.debug(f"File exists at {path}")
                        return path
                    else:
                        ...
                        for alt_path in alt_paths:
                            if os.path.exists(alt_path):
                                log.debug(f"File found at alternative path: {alt_path}")
                                return alt_path
    ...
    async def _upload_to_storage(
        self, user_id: str, content: bytes, filename: str, content_type: str
    ) -> Dict[str, Any]:
    ...
    async def _emit_progress(
        self, emitter, current_step: int, total_steps: int, step_description: str
    ) -> None:
    ...
    async def _emit_citation(self, emitter, document: str, source_name: str) -> None:
        """
    ...
    async def read_document(
        ...
        """
        log.info(
            f"read_document called with file_name={file_name}, __files__={__files__}"
        )
    ...
    async def _edit_block_internal(
        ...
        try:
            ...
            await self._emit_status(
                __event_emitter__, f"Block {block_id} updated", done=True
            )
    ...
    async def find_replace(
        ...
        """
        await self._emit_status(
            __event_emitter__, f"Finding and replacing text in {block_id}..."
        )

        try:
            file_name = self._clean_field_value(file_name, "")
            editor = await self._get_editor(__files__, file_name)
            result = editor.find_replace_in_block(
                block_id, find_text, replace_text, replace_all
            )
    ...
    async def insert_block(
        ...
        text: str = Field(..., description="Text content for the new paragraph."),
        ...
        try:
            ...
            await self._emit_status(
                __event_emitter__, f"Paragraph inserted{style_info}", done=True
            )
            ...
            self._log_tool_call(
                "insert_block",
                {
                    "ref_block": relative_to_block_id,
                    "position": position,
                    "text": text_preview,
                    **result,
                },
    ...
    # -------------------------------------------------------------------------
    # Table Cell Editing Tools (for bilingual NDAs)
    # -------------------------------------------------------------------------

    async def find_replace_in_cell(
        self,
        cell_id: str = Field(
            ...,
        ...
        find_text: str = Field(
            ...,
            description="The EXACT text to find within the cell. Must match precisely.",
        ),
        replace_text: str = Field(..., description="The text to replace it with."),
        file_name: str = Field(
            default="",
        ...
        """
        Find and replace text within a table cell. Use this for bilingual table-based NDAs.

        For bilingual documents, remember to edit BOTH language columns:
        - German column: Replace German text (e.g., "unverzüglich" → "ohne schuldhaftes Zögern")
        - English column: Replace English text (e.g., "immediately" → "without undue delay")

        The find_text must match EXACTLY (case-sensitive).

        Returns:
            On success: "✓ Replaced 'X' → 'Y' in cell_id"
            On failure: "✗ Error: ..."
        """
        await self._emit_status(
            __event_emitter__, f"Finding and replacing text in {cell_id}..."
        )
        ...
        try:
            ...
            if result["status"] == "success":
                await self._emit_status(
                    __event_emitter__, f"Replaced text in {cell_id}", done=True
                )
                self._log_tool_call(
                    "find_replace_in_cell",
                    {"cell_id": cell_id, "find": find_text, "replace": replace_text, **result},
                )
                return f"✓ Replaced '{find_text}' → '{replace_text}' in {cell_id}"
            else:
                await self._emit_status(
                    __event_emitter__, f"Warning: {result['message']}", done=True
                )
    ...
    async def get_edited_document(
        ...
        try:
            ...
            await self._emit_status(
                __event_emitter__, "Document state retrieved", done=True
            )
    ...
    async def generate_redline_document(
        ...
        try:
            ...
            await self._emit_progress(
                __event_emitter__, 1, total_steps, "Preparing document..."
            )

            file_name = self._clean_field_value(file_name, "")
            redline_author = self._clean_field_value(
                redline_author, self.valves.DEFAULT_REDLINE_AUTHOR
            )
            ...
            await self._emit_progress(
                __event_emitter__, 2, total_steps, "Saving modified document..."
            )
            ...
            await self._emit_progress(
                ...
                3,
                total_steps,
                "Creating tracked changes (this may take a moment)...",
            )
            ...
            redline_suffix = self.valves.REDLINE_SUFFIX
            redline_filename = f"{base_name}{redline_suffix}.docx"
            redline_path = os.path.join(
                os.path.dirname(modified_path), redline_filename
            )

            engine = XmlPowerToolsEngine()
            redline_bytes = engine.run_redline(
                redline_author, original_path, modified_path
            )
            ...
            await self._emit_progress(
                __event_emitter__, 4, total_steps, "Uploading redline document..."
            )
                ...
                await __event_emitter__(
                    {
                        ...
                        "data": {
                            ...
                            "document": [
                                f"Redline document with tracked changes by {redline_author}"
                            ],
            ...
            await self._emit_progress(
                ...
                5,
                total_steps,
                f"Complete! Redline ready: {redline_filename}",
            )
            ...
            if self.valves.LOG_LEVEL != "NONE":
                ...
                if logs_text:
                    await self._emit_citation(
                        __event_emitter__, logs_text, "Tool Execution Log"
                    )
