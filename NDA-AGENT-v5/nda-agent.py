...
class WordEditor:
    def __init__(self, file_path: str, __event_emitter__=None):
        ...
        self.tables = []           # List of Table objects in document order
        self.id_to_cell = {}       # Maps "cell_0_5_0" -> (table, row_idx, col_idx)
        self.cell_to_id = {}       # Maps (table_idx, row_idx, col_idx) -> cell_id
        self.table_metadata = {}   # Maps table_idx -> classification info
    ...
    def _map_document_blocks(self):
        ...
        for child in self.document.element.body:
            if isinstance(child, CT_P):
                ...
                # Also collect tables for cell-level mapping
    ...
    def _classify_tables(self):
        ...
        for idx, table in enumerate(self.tables):
            ...
            if rows > 10 and cols >= 2:
                # Calculate text per column, handling merged cells per-row
                col_text = {}
                has_merged = False
                ...
                # Need at least 2 columns with real content
                if len(text_cols) >= 2:
            ...
            self.table_metadata[idx] = classification
    ...
    def _detect_language_columns(self, table, text_cols: List[int]) -> Dict[int, str]:
        ...
        for col in text_cols:
            ...
            log.debug(f"Column {col}: de_score={de_score}, en_score={en_score} -> {lang_map[col]}")
    ...
    def _map_table_cells(self):
        ...
        """
        for table_idx, metadata in self.table_metadata.items():
            ...
            for row_idx, row in enumerate(table.rows):
                ...
                for col_idx, cell in enumerate(row.cells):
                    ...
                    # Skip merged cells (already mapped via another column)
                    if tc_id in seen_in_row:
                        continue
                    seen_in_row.add(tc_id)
                    ...
                    self.id_to_cell[cell_id] = (table, row_idx, col_idx)
                    self.cell_to_id[(table_idx, row_idx, col_idx)] = cell_id

        log.debug(f"Mapped {len(self.id_to_cell)} table cells across {len([m for m in self.table_metadata.values() if m['type']=='content'])} content tables")
    ...
    def read_document(self, event_emitter=None) -> dict:
        ...
        for table_idx, metadata in self.table_metadata.items():
            ...
            if metadata["type"] == "content":
                content_tables.append(table_idx)
        ...
        cells = []
        for cell_id, (table, row_idx, col_idx) in self.id_to_cell.items():
            cell = table.rows[row_idx].cells[col_idx]
            table_idx = self.tables.index(table)
            lang = self.table_metadata[table_idx]["language_columns"].get(col_idx)
        ...
        document_type = "bilingual_table" if content_tables else "paragraph_based"

        # Build summary with can_proceed logic
        summary = {
            "can_proceed": True,
    ...
    def insert_block(self, relative_to_block_id: str, position: str, new_block: dict):
        ...
        if relative_to_block_id.startswith("cell_"):
            # Parse cell_T_R_C format to get table index
            parts = relative_to_block_id.split("_")
            if len(parts) >= 2:
                table_idx = int(parts[1])
                # Find the table block ID
                table_block_id = None
                for block_id, element in self.id_to_element.items():
                    if isinstance(element, Table) and self.tables.index(element) == table_idx:
                        table_block_id = block_id
                        break
                if table_block_id:
                    log.debug(f"insert_block: Converted cell ID '{relative_to_block_id}' to table block '{table_block_id}'")
                    relative_to_block_id = table_block_id
                else:
                    raise ValueError(f"Could not find table block for cell '{relative_to_block_id}'.")
    ...
    # =========================================================================
    # TABLE CELL EDITING METHODS (for bilingual table-based NDAs)
    # =========================================================================

    def edit_cell(self, cell_id: str, new_text: str, preserve_formatting: bool = True):
        """
        Edit a table cell's text content.
        For bilingual NDAs, use find_replace_in_cell for surgical edits.
        """
        if cell_id not in self.id_to_cell:
            raise ValueError(f"Cell '{cell_id}' not found.")

        table, row_idx, col_idx = self.id_to_cell[cell_id]
        cell = table.rows[row_idx].cells[col_idx]
        ...
        if preserve_formatting and cell.paragraphs and cell.paragraphs[0].runs:
            ...
            # Clear first paragraph and add new content
            first_para.clear()
            new_run = first_para.add_run(new_text)
    ...
    def find_replace_in_cell(self, cell_id: str, find_text: str, replace_text: str):
        """
    ...
    def get_corresponding_cell(self, cell_id: str) -> Optional[str]:
        ...
        metadata = self.table_metadata.get(table_idx)
        ...
        if other_cols:
            ...
            if other_cell_id in self.id_to_cell:
    ...
    def get_cell_info(self, cell_id: str) -> Dict[str, Any]:
        ...
        cell = table.rows[row_idx].cells[col_idx]
        table_idx = self.tables.index(table)
        metadata = self.table_metadata[table_idx]
...
class Valves(BaseModel):
    """Configuration valves for the NDA Agent tool."""
    ...
    redline_author: str = Field(
        default="NDA Agent",
        description="Author name to use for tracked changes in the redline document."
    )
...
class Tools:
    ...
    # =========================================================================
    # TABLE CELL EDITING TOOL METHODS (for bilingual table-based NDAs)
    # =========================================================================
    ...
    async def edit_cell(
        ...
        new_text: str = Field(..., description="The new text content for the cell."),
        file_name: str = Field(
            default="",
            description="Name of the DOCX file (optional if file is uploaded).",
        ),
        ...
        """
        Edit the text content of a specific table cell IN MEMORY.
        Use this for bilingual table-based NDAs. Use 'save_document' to persist changes.
        """
        if __event_emitter__:
            await __event_emitter__(
                {
                    "type": "status",
                    "data": {
                        "description": f"Editing Cell {cell_id}",
                        "done": False,
                        "hidden": False,
                    },
                }
        ...
        try:
            ...
            editor = await self._get_editor(__files__, clean_file_name)
            result = editor.edit_cell(cell_id, new_text)
            result["message"] += " Change is in memory. Use 'save_document' to persist."
            return result
        except Exception as e:
            return {"error": traceback.format_exc(), "status": "failed"}
    ...
    async def find_replace_in_cell(
        self,
        cell_id: str = Field(
            ..., description="The ID of the cell to edit (e.g., 'cell_0_5_0')."
        ),
        find_text: str = Field(..., description="The text to find and replace."),
        ...
        """
        Find and replace text within a specific table cell IN MEMORY.
        Preserves formatting. For bilingual NDAs, remember to edit BOTH language columns.
        Use 'save_document' to persist changes.
        """
        if __event_emitter__:
            await __event_emitter__(
                {
                    "type": "status",
                    "data": {
                        "description": f"Find/Replace in Cell {cell_id}",
                        "done": False,
                        "hidden": False,
                    },
                }
    ...
    async def get_cell_info(
        self,
        cell_id: str = Field(
            ..., description="The ID of the cell to get info for (e.g., 'cell_0_5_0')."
        ),
        file_name: str = Field(
            default="",
            description="Name of the DOCX file (optional if file is uploaded).",
        ...
        """
        Get detailed information about a specific table cell, including its corresponding
        cell in the other language column (for bilingual documents).
        """
        if __event_emitter__:
            await __event_emitter__(
                {
                    "type": "status",
                    "data": {
                        "description": f"Getting info for Cell {cell_id}",
                        "done": False,
                        "hidden": False,
                    },
                }
        ...
        try:
            ...
            editor = await self._get_editor(__files__, clean_file_name)
            result = editor.get_cell_info(cell_id)
            return result
        except Exception as e:
            return {"error": traceback.format_exc(), "status": "failed"}
    ...
    async def generate_redline_document(
        ...
        try:
            ...
            # Derive redline filename from the ORIGINAL file name (not output_name)
            original_name = os.path.basename(original_path)
            name_part, ext = os.path.splitext(original_name)
            # Remove any file ID prefix if present (format: {uuid}_{filename})
            if "_" in name_part and len(name_part.split("_")[0]) == 36:
                name_part = "_".join(name_part.split("_")[1:])
            redline_suffix = self.valves.redline_suffix if hasattr(self, 'valves') else "_redlined"
            redline_filename = f"{name_part}{redline_suffix}{ext}"
            ...
            # Use valve default if author not provided, otherwise use provided author
            author_name = clean_redline_author
            if author_name == "Editor" and hasattr(self, 'valves'):
                author_name = self.valves.redline_author
            redline_bytes_tuple = wrapper.run_redline(
                author_name, original_path, modified_path
            )
