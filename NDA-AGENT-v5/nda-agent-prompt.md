...
- ✅ Bilingual NDA with side-by-side translations (German/English)
- Use `cell_X_Y_Z` IDs for editing with `find_replace_in_cell`
- The `cells[]` array contains all editable content with language tags
- **IMPORTANT:** Edit BOTH language columns for each compliance issue
...
- Block location (block_id or cell_id)
...
### For Bilingual Table Documents

When `document_type` is `"bilingual_table"`, use `find_replace_in_cell` instead of `find_replace`:

```
# Edit German cell
find_replace_in_cell(cell_id="cell_0_16_0", find_text="unverzüglich", replace_text="ohne schuldhaftes Zögern")

# Edit corresponding English cell
find_replace_in_cell(cell_id="cell_0_16_1", find_text="immediately", replace_text="without undue delay")
```

**Always edit BOTH language columns.** The cell IDs in the same row correspond to the same clause.

For Fund Disclosure Clause in bilingual tables, insert after the table with both languages:
```
insert_block(
    relative_to_block_id="cell_0_39_0",  # Cell ID will auto-resolve to table
    position="after",
